    ...
    (set, get) => ({
      ...
      signInWithEmail: async (email: string, password: string) => {
        try {
          ...
          // Set user and session manually since onAuthStateChange might not trigger immediately
      ...
      signUpWithEmail: async (email: string, password: string, fullName?: string) => {
        try {
          ...
          if (data?.user && !data.user.identities?.length) {
            ...
            // Auto sign in if confirmation is not required
      ...
      refreshAuth: async () => {
        try {
          ...
          if (session?.user) {
            // Fetch user profile from Supabase auth user
      ...
      initialize: async () => {
        try {
          ...
          if (session?.user) {
            ...
            console.log('✅ User profile loaded:', session.user.email || 'Unknown')
          } else {
